...
def save_to_csv() -> None:
    if "titanic_data.csv" not in [
        file.name for file in Path(config_file["path"]["DATA"]).iterdir()
    ]:
        new_data = load_data_testing()
        new_data.to_csv(
            Path(config_file["path"]["DATA"]) / "titanic_data.csv", index=False
        )
    else:
        pass
...
def load_data(split=False, data_: str = "train") -> pd.DataFrame:
    ...
    titanic_data.drop(
        labels=config_file["attributes"]["drop_variables"], axis=1, inplace=True
    )

    if split:
        X_train, X_test, y_train, y_test = train_test_split(
            titanic_data.drop(
                config_file["base"]["target_column"], axis=1
            ),  # predictors
